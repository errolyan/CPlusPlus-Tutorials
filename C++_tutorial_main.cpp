...
#include <opencv2/imgproc/imgproc.hpp>//add by hjx
#include <windows.h>
#include <net.h>
#include <stdlib.h>
#include <time.h>
	...
	for (int row = 0; row < score.h; row++) {
		for (int col = 0; col < score.w; col++) {
			if (*p > threshold[0]) {
				...
				bbox.x1 = round((stride*col + 1) / scale);//”ˆ5Ω¯“ª£¨Ω¸À∆Œ™’˚ ˝
...
void mtcnn::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_) {
	...
	for (vector<Bbox>::iterator it = firstBbox_.begin(); it != firstBbox_.end(); it++) {
		if ((*it).exist) {
			...
			//mod by hjx@20180126 ncnn update
...
int main(int argc, char** argv)
{
	...
	//CV_LOAD_IMAGE_COLOR  º÷’◊™ªªµΩ≤ …´£¨‘Ÿ∑µªÿ
	//cv::Mat cv_img = cv::imread(imagepath, CV_LOAD_IMAGE_COLOR);
	cv::Mat cv_img = cv::imread(imagepath, IMREAD_COLOR);
	if (cv_img.empty())
	{
	...
	{
		float r, g, b;
		float * ptr = (float*)ncnn_img.data;
		r = ptr[300 + 300 * cv_img.cols + 0 * cv_img.cols*cv_img.rows];
		g = ptr[300 + 300 * cv_img.cols + 1 * cv_img.cols*cv_img.rows];
		b = ptr[300 + 300 * cv_img.cols + 2 * cv_img.cols*cv_img.rows];
		printf(" r %f g %f b %f!\n", r, g, b);
	}
	...
	for (vector<Bbox>::iterator it = finalBbox.begin(); it != finalBbox.end(); it++, index++) {
		if ((*it).exist) {
			printf("Box %d socre %f exist\n.", index, it->score);
			rectangle(cv_img, Point((*it).x1, (*it).y1), Point((*it).x2, (*it).y2), Scalar(0, 0, 255), 2, 8, 0);
			//cv::Rect rect=Rect(Point((*it).x1, (*it).y1), Point((*it).x2, (*it).y2));
			//rectangle(cv_img, cvPoint((*it).x1, (*it).y1), cvPoint((*it).x2, (*it).y2) , Scalar(0,0,255), 2,8,0);
			for (int num = 0; num < 5; num++)circle(cv_img, Point((int)*(it->ppoint + num), (int)*(it->ppoint + num + 5)), 3, Scalar(0, 255, 255), -1);
		}
		else
			printf("Box %d socre %f not exist\n.", index, it->score);
	}
